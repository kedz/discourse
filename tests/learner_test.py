...
from discourse.learners import DiscourseSequenceModel
import numpy as np

class TestLearnerLoss:
    ...
    def zero_one_loss_test(self):
        ...
        assert self.dsm.zero_one_loss(self.y, self.y_hat2, False) == 0

    def hamming_node_loss_test(self):
        assert self.dsm.hamming_node_loss(self.y, self.y_hat1, False) == 2
        assert self.dsm.hamming_node_loss(self.y, self.y_hat2, False) == 0

    def hamming_edge_loss_test(self):
        assert self.dsm.hamming_edge_loss(self.y, self.y_hat1, False) == 3
        assert self.dsm.hamming_edge_loss(self.y, self.y_hat2, False) == 0

class TestLearnerAndModel:
...
"""
    ...
    def build_hamming_node_potentials_test(self):
        inst = NGramDiscourseInstance(self.doc, {'debug': True}, None, 2)
        hypergraph = inst.hypergraph()
        y = inst.gold_transitions()
        w = np.array([0])
        dsm = DiscourseSequenceModel(loss_function='hamming-node')
    ...
    def build_hamming_edge_potentials_test(self):
        inst = NGramDiscourseInstance(self.doc, {'debug': True}, None, 2)
        hypergraph = inst.hypergraph()
        y = inst.gold_transitions()
        w = np.array([0])
        dsm = DiscourseSequenceModel(loss_function='hamming-edge')
        dsm._vec.fit_transform({'DEBUG': 1})
        
        potentials = dsm._build_hamming_potentials(hypergraph, inst, y, w)

        for i, edge in enumerate(hypergraph.edges):
            assert dsm.hamming_edge_loss(y, [edge.label]) == potentials[edge]
